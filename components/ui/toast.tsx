...
import { toastEntrance } from '@/components/motion'
...
function ToastItem({ toast, onDismiss }: { toast: Toast; onDismiss: () => void }) {
  ...
  return (
    <motion.div
      ...
      {...toastEntrance}
      className={cn(
        'pointer-events-auto flex items-start gap-3 p-4 rounded-2xl bg-card border shadow-2xl max-w-sm elevation-xl',
        BORDERS[toast.type]
      )}
    >
      <motion.div
      ...
      <button
        onClick={onDismiss}
        className="shrink-0 p-0.5 rounded-lg hover:bg-secondary transition-colors"
